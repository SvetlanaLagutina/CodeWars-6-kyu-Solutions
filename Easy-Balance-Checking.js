...
function balance(book) {
    let lns = book.split(/\r?\n/).map(l => l.replace(/[^a-zA-Z0-9-_\. ]/g, '')).filter(e => e); 
    let b = parseFloat(lns[0]); 
    let arr = [];
    let parsed = lns.map((l, i) => {
      let nl = l.split(" "), pf = parseFloat(nl[2]);
      if(nl.length !== 1) {
        arr.push(pf);
        let nb =  b -= pf;
    ...
    let sum = arr.reduce((a, n) => a+n, 0).toFixed(2), avg = sum/arr.length;
